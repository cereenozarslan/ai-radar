...
def test_init_db_creates_items_table(tmp_path):
    ...
    assert columns == {
        ...
        "signal_reason", "is_read", "is_saved", "popularity", "is_online", "fetched_at",
    }
...
def test_init_db_migrates_old_database_missing_new_columns(tmp_path):
    ...
    row = conn.execute(
        "SELECT title, image_url, is_read, is_saved, popularity, is_online FROM items WHERE url = ?",
        ("https://example.com/eski",),
    ).fetchone()
    conn.close()

    assert {"image_url", "is_read", "is_saved", "popularity", "is_online"} <= columns
    assert row == ("eski kayıt", None, 0, 0, None, None)
